...
export default function ExpiredJobList() {
  ...
  const fetchJobs = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/jobs/listexpiredjobs?page=0&size=1000`, {
        method: "GET",
        headers: {
        ...
        <div className="w-full md:w-3/4">
          <div className="space-y-4">
            {paginatedJobs.map((job) => (
              <div
                key={job.id}
                className={`bg-white p-4 rounded-lg shadow ${isJobExpired(job.expiryDate) ? "" : ""}`}
              >
